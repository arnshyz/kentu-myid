  ...
  if (req.method === 'POST') {
    ...
    if (crm) {
      try {
        const r = await fetch(crm, { method:'POST',
          headers: { 'Content-Type':'application/json', 'X-CRM-Token': process.env.CRM_WEBHOOK_TOKEN || '' },
          body: JSON.stringify({ admin_action:'UPDATE_MEMBER', ...body })
        });
    ...
    const i = members.findIndex(m => m.email === body.email);
    if (i >= 0) members[i] = { ...members[i], tier: body.tier || members[i].tier, verified: (body.verified ?? members[i].verified) };
